...
[source,shell]
----
podman run --rm --network=bridge --name my-postgres \
  -p 5432:5432 \
...
[source,shell]
----
POSTGRESQL_CONTAINER_IP=$(podman inspect my-postgres | jq -r '.[].NetworkSettings.Networks[].IPAddress')

podman run --rm --network=bridge -p 8080:8080 -p 9990:9990 \
    -e POSTGRESQL_USER=postgres \
    -e POSTGRESQL_PASSWORD=admin \
    -e POSTGRESQL_HOST=$POSTGRESQL_CONTAINER_IP \
